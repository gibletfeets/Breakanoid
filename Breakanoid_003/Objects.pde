...
class paddleObject
{
  ...
  void display()
  {
    ...
    if (gameBall != null)  
    {
      ...
      float scalingFactor = map(relative.mag(),0,435,0,1);
      relative.set((size.x/2-3)*cos(angle),4*sin(angle));
      relative.y *= scalingFactor;
      ellipse(pos.x+relative.x,pos.y+relative.y,3,3);
